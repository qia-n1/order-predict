...
MODEL_DIR = ROOT / "models" / "xgboost_hourly_v3_may_jun"
...
def main() -> None:
    ...
    stage1 = pd.read_csv(MODEL_DIR / "stage1_history.csv")

    fig = plt.figure(figsize=(18, 12), dpi=180)
    gs = fig.add_gridspec(4, 4, height_ratios=[0.85, 1.35, 1.35, 1.45], hspace=0.62, wspace=0.34)

    fig.text(0.03, 0.975, "V3 全量测评指标图", fontsize=24, weight="bold", ha="left", va="top")
    subtitle = (
        f"数据：Uber_2014_MayJun_features.csv | 划分：train_ratio=0.7, "
        f"valid_ratio=0.15, test_start_date=2014-06-24 | Stage1={metrics['stage1_model']} | "
        f"embedding={metrics['embedding_dim']} | features={metrics['feature_count']} | freq={metrics['freq']}"
    )
    ...
    cards = [
        ...
        ("Best Stage1", f"Epoch {metrics['best_stage1_epoch']}", f"Valid MAE {metrics['best_stage1_valid_mae']:.4f}", BLUE),
    ]
    ...
    ax.bar(["Train", "Valid", "Test"], rows, color=[BLUE, CYAN, GREEN], alpha=0.85)
    ax.set_title("Stage2 样本规模", fontsize=13)
    ax.set_ylabel("Rows")

    ax = fig.add_subplot(gs[3, 0])
    ax.plot(stage1["epoch"], stage1["train_mae"], color=BLUE, label="Train MAE")
    ax.plot(stage1["epoch"], stage1["valid_mae"], color=CYAN, label="Valid MAE")
    ax.set_title("Stage1 MAE 曲线", fontsize=13)
    ax.set_xlabel("Epoch")
    ax.legend(frameon=False)
    ...
    ax.set_title("Stage2 XGBoost 曲线", fontsize=13)
