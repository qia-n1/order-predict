...
MODEL_DIR = ROOT / "models" / "xgboost_hourly_v1_may_jun"
...
def main() -> None:
    ...
    fig.text(0.03, 0.975, "V1 全量测评指标图", fontsize=24, weight="bold", ha="left", va="top")
    subtitle = (
        f"数据：Uber_2014_MayJun_features.csv | 划分：train_ratio=0.7, "
        f"valid_ratio=0.15, test_start_date=2014-06-24 | H3={metrics['h3_resolution']} | freq={metrics['freq']}"
    )
    ...
    ax.set_ylabel("Rows")
    ax2 = ax.twinx()
    ax2.plot(split_labels, zeros, color=AMBER, marker="o", linewidth=2.5, label="Zero ratio")
    ...
    valid_rmse = evals["valid"]["rmse"]
    train_mae = evals["train"]["mae"]
    valid_mae = evals["valid"]["mae"]
    ax.plot(train_rmse, color=BLUE, label="Train RMSE")
    ax.plot(valid_rmse, color=CYAN, label="Valid RMSE")
    ax.plot(train_mae, color=GREEN, label="Train MAE")
    ax.plot(valid_mae, color=AMBER, label="Valid MAE")
